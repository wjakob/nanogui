...
public:
     ...
     *     portability reasons).  For example, set this to 4 and ``glMinor`` to
     *     1 for a forward compatible core OpenGL 4.1 profile.  Requesting an
     ...
     *     the value must correspond to a forward compatible core profile (for
     *     portability reasons).  For example, set this to 1 and ``glMajor`` to
     *     4 for a forward compatible core OpenGL 4.1 profile.  Requesting an
     *     invalid profile will result in no context (and therefore no GUI)
     *     being created.
     */
...
public:
     ...
     *
     * In this case, override \ref Screen and call \ref Screen::initialize()
     * with a pointer to an existing \c GLFWwindow instance.
     *
